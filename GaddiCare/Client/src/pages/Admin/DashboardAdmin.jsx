...
const DashboardAdmin = () => {
  ...
  useEffect(() => {
    if (users && allWorkshops && pendingWorkshops && bookings) {
      ...
      const vehicleOwners = users.filter(user => user.userType === 'vehicleOwner');
      
      const todayNewUsers = vehicleOwners.filter(user => {
  ...
  }, [users, allWorkshops, pendingWorkshops, bookings]);
  ...
  const calculateChartData = () => {
    ...
    for (let month = 0; month < 12; month++) {
      ...
      if (month <= currentMonth) {
        const usersInMonth = vehicleOwners.filter(user => {
    ...
    for (let month = 0; month < 12; month++) {
      ...
      const monthlyUsers = vehicleOwners.filter(user => {
        const userDate = new Date(user.createdAt);
        return userDate >= monthStart && userDate <= monthEnd;
      });
  ...
  const vehicleOwners = users?.filter(user => user.userType === 'vehicleOwner') || [];
  const totalUsers = vehicleOwners.length || 0;
